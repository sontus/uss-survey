    ...
    <div class="container">
        <div class="row mainView">
            <h5 class="text-center mt-3 mb-2 ">
                <span class="border border-dark p-1">Production Period-2</span>
            </h5>
            ...
            <div class="col-sm-6 col-md-6">
                ...
                <div class="buttons d-flex  justify-content-between">
                    <button type="button" class="pageChangeBtn prevButton" pageNo="1"> < Previous
                        page
                    </button>
                ...
                <div class=" questionArea">
                    {{-- pages --}}
                    @include('pages.production_tow.page_one')
                </div>
                {{-- </div> --}}
            </div>
        </div>
...
@push('page-js')
    <script>
        var period = "Production-Period-2";
        ...
        function pageChange(pageNo) {
            ...
            switch (pageNo) {
                case 1:
                    qArea.html(`@include('pages.production_tow.page_one')`);
                    break;
                case 2:
                    qArea.html(`@include('pages.production_tow.page_two')`);
                    break;
                case 3:
                    qArea.html(`@include('pages.production_tow.page_three')`);
                    break;
                case 4:
                    qArea.html(`@include('pages.production_tow.page_four')`);
                    break;
                case 5:
                    qArea.html(`@include('pages.production_tow.page_five')`);
                    break;
                case 6:
                    qArea.html(`@include('pages.production_tow.page_six')`);
                    break;
                case 7:
                    qArea.html(`@include('pages.production_tow.page_seven')`);
                    break;
                case 8:
                    qArea.html(`@include('pages.production_tow.page_eight')`);
                    break;
                case 9:
                    qArea.html(`@include('pages.production_tow.page_nine')`);
                    break;
                case 10:
                    qArea.html(`@include('pages.production_tow.page_ten')`);
                    break;
                case 11:
                    qArea.html(`@include('pages.production_tow.page_eleven')`);
                    break;
                case 12:
                    qArea.html(`@include('pages.production_tow.page_twelve')`);
                    break;
                case 13:
                    qArea.html(`@include('pages.production_tow.page_thirteen')`);
                    break;
                case 14:
                    qArea.html(`@include('pages.production_tow.page_fourteen')`);
                    break;
                case 15:
                    qArea.html(`@include('pages.production_tow.page_fifteen')`);
                    break;
                case 16:
                    qArea.html(`@include('pages.production_tow.page_sixteen')`);
                    break;
                case 17:
                    qArea.html(`@include('pages.production_tow.page_seventeen')`);
                    break;
                case 18:
                    qArea.html(`@include('pages.production_tow.page_eighteen')`);
                    break;
                case 19:
                    qArea.html(`@include('pages.production_tow.page_nineteen')`);
                    break;
                case 20:
                    qArea.html(`@include('pages.production_tow.page_twenty')`);
                    break;
            }
        }
        ...
        $(".ans_input").blur(function () {
            ...
            let corAns = sessionStorage.getItem("productionTowCorrect");
            if (corAns == null) {
                corAns = 0;
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data === true) {
                        inputRow.parent('td').css({"background-color": "green"});
                        inputRow.css('color', '#fff');
                        corAns = parseInt(corAns) + 1;
                        $('.correctAns').text(corAns);
                        sessionStorage.setItem("productionTowCorrect", corAns);
                    } else {
                        // inputRow.parent('td').css({"background-color": "red"});
        ...
        function saveAnswer(pageNumber, ansData) {
            ...
            $.ajax({
                ...
                'type': 'post',
                'dataType': 'json',
                'data': {pageNum: pageNumber, pageData: ansData},
                'cache': false,
                success: function (data) {
        ...
        function countdown(elementName, minutes, seconds) {
            ...
            function updateTimer() {
                ...
                if (msLeft < 1000) {
                    if (minutes > 0) {
                        collectAllAns();
                        // sessionStorage.setItem("productionTowTimeUp", 'up');
                        @php
                            session()->put('productionTowTimeUp','up');
                        @endphp
                    }
                } else {
        ...
        function saveFinalAnswer(getPeriod, allAnsData) {
            ...
            $.ajax({
                ...
                'type': 'post',
                'dataType': 'json',
                'data': {period: getPeriod, pageData: allAnsData},
                'cache': false,
                success: function (data) {
                    // console.log('Working');
                    window.location = 'shortcuts-discovered-in-this-period-two';
        ...
        //#### <--Number increament by ArrowUp/down button disable code start--> ####

        // First way
        // const numberInputs = document.querySelectorAll('.ans_input');
